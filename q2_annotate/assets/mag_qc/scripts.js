$(document).ready(function () {
  ...
  function renderChart() {
    ...
    if (scalingMethod === 'length') {
      ...
      maxRightPlotValue = d3.max(mags, d => {
        ...
        Object.values(contigsMap).forEach(contigList => {
          totalContigCount += contigList.length;
        });
    ...
    rowsEnter.each(function (d) {
      ...
      if (iconClass) {
        iconFo.append('xhtml:div').style('width', '100%').style('height', '100%').style('display', 'flex').style('align-items', 'center').style('justify-content', 'center').html(`<i class="${iconClass} ${colorClass} quality-icon"></i>`);
      }
    });
    ...
    rowsEnter.each(function (d) {
      ...
      if (scalingMethod === 'length') {
        ...
        Object.values(contigsMap).forEach(contigList => {
          totalIndividualContigsInMAG += (contigList || []).length;
        });
  ...
  d3.json("data.json?v=" + Date.now()).then(function (loadedData) {
    vizData = loadedData;
  ...
  function renderMAGGCKDEPlot(d) {
    ...
    window.requestAnimationFrame(() => {
      ...
      const {
        points: kdeData,
        maxDensity: calculatedMaxDensity
      } = calculateKDE(allMAGContigs, bandwidth, window.gcKDEIsWeighted);
