...
def _validate_parameters(metadata, remove_empty, where, exclude_ids):
    if not metadata and not remove_empty:
        raise ValueError(
            'Please specify parameters "--m-metadata-file" or '
            '"--p-remove-empty"  to filter accordingly.'
        )

    if where and not metadata:
        raise ValueError(
            'The parameter "--p-where" can only be specified in '
    ...
    if exclude_ids and not metadata:
        raise ValueError(
            'The parameter "--p-exclude-ids" can only be '
        ...
        for inner_id, file_fp in inner_dict.items():
            with open(file_fp, "r") as file:
                ...
                if not second_line or not third_line:
                    ...
                    # "root"
                    if len(columns) > 5 and columns[5] in ["unclassified", "root"]:
    ...
    for outer_id, inner_dict in file_dict.items():
        for inner_id, file_fp in inner_dict.items():
            if inner_id in ids_to_keep:
                if outer_id:
                    os.makedirs(os.path.join(str(fmt), outer_id), exist_ok=True)
                duplicate(
...
def _filter_kraken2_results_by_metadata(
    ...
    if metadata:
        ...
        if not (set(selected_ids) - ids_to_keep):
            print(
                f"IDs {', '.join(sorted(set(selected_ids) - ids_to_keep))} "
...
def filter_kraken2_results(
    ...
    """
    if metadata is None and remove_empty is False and abundance_threshold is None:
        msg = (
    ...
    if set(reports.view(Kraken2ReportDirectoryFormat).file_dict().keys()) != set(
        outputs.view(Kraken2OutputDirectoryFormat).file_dict().keys()
    ):
        msg = (
    ...
    if metadata is not None or remove_empty:
        ...
        post_md_reports, post_md_outputs = _filter_kraken2_results_by_metadata(
            reports, outputs, metadata, where, exclude_ids, remove_empty
        )
    else:
    ...
    (abun_f_outputs,) = _align_outputs_with_reports(post_md_outputs, abun_f_reports)
...
def _filter_kraken2_reports_by_abundance(
    ...
    for report_filename, report in reports.reports.iter_views(Kraken2ReportFormat):
...
def _report_df_to_tree(report: pd.DataFrame) -> tuple[TreeNode | None, TreeNode | None]:
    """
...
def _write_report_dfs(node: TreeNode, report: pd.DataFrame) -> None:
    ...
    children.sort(key=lambda n: n._kraken_data["n_frags_covered"], reverse=True)
...
def _align_outputs_with_reports(
    ...
    for sample_id, report_path in report_sample_map.items():
        ...
        _align_single_output_with_report(output, report, aligned_outputs, sample_id)
