...
class TestFilterKrakenReports(TestPluginBase):
    ...
    def setUpClass(cls):
        ...
        cls.report_mags_unclassified_missing_frac = Kraken2ReportDirectoryFormat(
            instance.get_data_path("reports-mags-unclassified-missing-frac"), "r"
        )
        cls.report_mags_unclassified_with_root = Kraken2ReportDirectoryFormat(
            instance.get_data_path("reports-mags-unclassified-root"), "r"
        )
        ...
        cls.metadata_df = pd.read_csv(
            instance.get_data_path("metadata/metadata.tsv"), sep="\t", index_col="id"
        )

        cls.metadata1 = qiime2.Metadata(cls.metadata_df)

        cls.metadata_df.drop("8894435a-c836-4c18-b475-8b38a9ab6c6b", inplace=True)
        cls.metadata2 = qiime2.Metadata(cls.metadata_df)

    def test_find_empty_reports(self):
        empty_reports = _find_empty_reports(file_dict={"": self.file_dict_report_mags})
        self.assertEqual(empty_reports, {"8894435a-c836-4c18-b475-8b38a9ab6c6b"})
    ...
    def test_filter_kraken_reports_error(self):
        with self.assertRaisesRegex(ValueError, "No IDs remain after filtering."):
            _filter_kraken2_results_by_metadata(
    ...
    def test_missing_metadata_and_remove_empty(self):
        with self.assertRaisesRegex(ValueError, r"--m-metadata-file.*--p-remove-empty"):
            _validate_parameters(
                metadata=None, remove_empty=False, where=None, exclude_ids=False
            )

    def test_where_without_metadata(self):
        with self.assertRaisesRegex(ValueError, r"--p-where.*--m-metadata-file"):
            _validate_parameters(
                metadata=None, remove_empty=True, where=True, exclude_ids=False
            )

    def test_exclude_ids_without_metadata(self):
        with self.assertRaisesRegex(ValueError, r"--p-exclude-ids.*--m-metadata-file"):
            _validate_parameters(
                metadata=None, remove_empty=True, where=None, exclude_ids=True
            )
    ...
    def test_validate_ids_error(self):
        ...
        msg = (
            r"reports\: \{'3b72d1a7-ddb0-4dc7-ac36-080ceda04aaa'\}.*\n.*outputs\: "
...
class TestAbundanceFilter(TestPluginBase):
    ...
    def setUp(self):
        ...
        curated_report_fp = self.get_data_path("filter/reports/sample-1.report.txt")
        curated_report = Kraken2ReportFormat(curated_report_fp, mode="r")
        self.curated_report = curated_report.view(pd.DataFrame)

        real_report_fp = self.get_data_path("filter/reports/SRR17001003.report.txt")
    ...
    def test_curated_report_to_tree(self):
        ...
        # find node 1767
        nodes_list = list(
            root.find_by_func(lambda n: n._kraken_data["taxon_id"] == 1767)
        )
        ...
        self.assertEqual(
            set(node_1767_sibling_ids), set([1138383, 701042, 1764, 339268, 2775496])
        )
    ...
    def test_real_report_to_tree(self):
        ...
        # find node 2732008
        nodes_list = list(
            root.find_by_func(lambda n: n._kraken_data["taxon_id"] == 2732008)
        )
        ...
        self.assertEqual(set(node_2732008_children_ids), set([2732529, 3044425]))
    ...
    def test_trim_curated_tree(self):
        ...
        def find_by_name(name, root):
            nodes_list = list(
                root.find_by_func(lambda n: n._kraken_data["name"].strip() == name)
            )
        ...
        filtered_root = _trim_tree_dfs(
            root.copy(deep=True), abundance_threshold=0.003, total_reads=total_reads
        )
        ...
        self.assertIsNone(find_by_name("Mycobacterium marseillense", filtered_root))

        self.assertIsNotNone(find_by_name("Mycobacterium gordonae", root))
        self.assertIsNone(find_by_name("Mycobacterium gordonae", filtered_root))

        # some trimmed node's children should be gone
        self.assertIsNotNone(find_by_name("Mycobacterium tuberculosis", root))
        self.assertIsNone(find_by_name("Mycobacterium tuberculosis", filtered_root))

        # a node with no n_frags_assigned whose children were all removed
        # is removed
        self.assertIsNotNone(find_by_name("Mycobacterium tuberculosis complex", root))
        self.assertIsNone(
    ...
    def test_curated_report_round_trip(self):
        ...
        round_tripped_report.sort_values("taxon_id", inplace=True, ascending=False)
        curated_report = self.curated_report.sort_values("taxon_id", ascending=False)

        columns = ["n_frags_covered", "n_frags_assigned", "taxon_id", "name", "rank"]
    ...
    def test_filter_kraken2_reports_by_abundance(self):
        ...
        num_input_reports = len(
            list(self.reports.reports.iter_views(Kraken2ReportFormat))
        )
        num_output_reports = len(
            list(filtered_reports.reports.iter_views(Kraken2ReportFormat))
        )
...
class TestOutputReportAlignment(TestPluginBase):
    ...
    def setUp(self):
        ...
        reports_fp = self.get_data_path("filter/output-report-alignment/reports/")
        self.reports = Kraken2ReportDirectoryFormat(reports_fp, mode="r")

        outputs_fp = self.get_data_path("filter/output-report-alignment/outputs/")
    ...
    def test_align_single_output_with_report(self):
        ...
        self.assertEqual(set(report_df["taxon_id"]), set(output_df["taxon_id"]))
    ...
    def test_align_outputs_with_reports(self):
        ...
        """
        aligned_outputs = _align_outputs_with_reports(self.outputs, self.reports)
        ...
        self.assertEqual(
            set(sample1_report_df["taxon_id"]), set(sample1_output_df["taxon_id"])
        )
        self.assertEqual(
            set(sample2_report_df["taxon_id"]), set(sample2_output_df["taxon_id"])
        )
    ...
    def test_errors_on_mismatched_sample_ids(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with self.assertRaisesRegex(ValueError, "sample IDs"):
                self.filter_kraken2_results(mag_reports, mag_outputs, remove_empty=True)
