...
from q2_types.kraken2 import BrackenDBDirectoryFormat, Kraken2ReportDirectoryFormat
...
class TestBracken(TestPluginBase):
    ...
    def test_estimate_bracken_with_unclassified(self, p1, p2, p3):
        kraken_reports = Kraken2ReportDirectoryFormat(
            self.get_data_path("bracken-report-with-unclassified/" "kraken-reports"),
            "r",
        )
        bracken_db = BrackenDBDirectoryFormat()

        table = pd.read_csv(
            self.get_data_path("bracken-report-with-unclassified/samples-merged.csv"),
            index_col=0,
        )
        ...
        exp_table = pd.read_csv(
            self.get_data_path(
                "bracken-report-with-unclassified/" "samples-merged-corrected.csv"
            ),
            index_col=0,
        )
        exp_table.index = pd.Index(exp_table.index, name="sample_id")
        ...
        exp_taxonomy = pd.read_csv(
            self.get_data_path("bracken-report-with-unclassified/" "taxonomy.csv"),
            index_col=0,
        )
